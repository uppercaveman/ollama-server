...
for TARGETARCH in ${BUILD_ARCH}; do
    ...
    docker cp builder-$TARGETARCH:/go/src/github.com/uppercaveman/ollama-server/dist/linux-$TARGETARCH ./dist
    if echo ${TARGETARCH} | grep "amd64" > /dev/null; then
        docker cp builder-$TARGETARCH:/go/src/github.com/uppercaveman/ollama-server/dist/linux-$TARGETARCH-rocm ./dist
    fi
